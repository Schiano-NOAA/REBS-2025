...
The profile on the assumption of $lnR_0$, which sets the initial scale of the population) demonstrates the data and current model specification support value of $lnR_0$ greater than 6.25, with a steep increase in likelihood difference as $lnR_0$ goes lower. The is the expectant increase in scale across increasing $lnR_0$ values for both initial and ending spawning output. The final spawning output goes down quicker than the initial spawning output, leading to a lower stock status. 
