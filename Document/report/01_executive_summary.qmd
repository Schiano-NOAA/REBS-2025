...
The model estimated uncertainty ($\sigma$) around the 2025 spawning output for is `r round(model_output$Pstar_sigma,2)`. The uncertainty around the OFL is `r round(model_output$OFL_sigma,2)`. These values underestimate the overall uncertainty as they do not incorporate the model structural uncertainty and do not account for any time-varying dynamics other than recruitment. The estimated uncertainty values are higher than the Category 1 default of 0.5, so all projections will use the estimated ($\sigma$).
