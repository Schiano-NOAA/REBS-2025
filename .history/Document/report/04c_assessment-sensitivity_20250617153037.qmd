...
The component likelihoods show a some conflict in the length and age data, with lengths pulling for higher $M$ values and ages for lower $M$ values (@piner_panel_NatM_uniform_Mal_GP_1). As noted when building the reference model, the reference model value for male $M$ is the lowest support by the age data, and was chosen to avoid the fast increasing scale estimates. 
