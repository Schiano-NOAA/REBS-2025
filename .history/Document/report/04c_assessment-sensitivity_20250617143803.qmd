...
The profile on the assumption of male $M$ (while still estimating female $M$) demonstrates the data and current model specification support a higher value of male $M$ than what is being used in the reference model (@fig-likeprof-M-parm). The is the expectant increase in scale across increasing $lnR_0$ values for both initial and ending spawning output. The final spawning output goes down quicker than the initial spawning output, leading to a lower stock status. Even at very low $lnR_0$ values, the stock does not drop below the target reference stock status.
