...
#| message: false
#| results: asis
#| tbl-cap: "Recent trend (last 10 years) in the overfishing limits (OFL), the acceptable biological catches (ABCs), the annual catch limits (ACLs), and the total dead catch (landings + discards) all in metric tons (mt) for the Rougheye/Blackspotted complex."

cat("\\begin{landscape}")

# Read in CSV, skipping the first 2 header rows
GMT_filtII<- read.csv(here::here("Document","report", "tables_4_doc", "management-performance_es.csv"))

recent_management_tableII <- recent_management_table |>
  full_join(GMT_filtII, by = "Year") |>
